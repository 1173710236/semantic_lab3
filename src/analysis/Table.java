...
public class Table
{
	...
	{
		this.dfa = new DFAset();
		DFA state0 = new DFA(0);
		state0.addNewDerivation(new ProS(getDerivation("P'").get(0),"#",0));  // 首先加入S'->·S,#
		for(int i = 0;i < state0.set.size();i++)
		{
			ProS lrd = state0.set.get(i);
			if(lrd.index < lrd.d.list.size())  // 非规约状态
			{
				...
				{
					ArrayList<Pro> dA = getDerivation(A);
					for(int j=0,length1=dA.size();j<length1;j++)
					{
						for(String f:firstB)
						{
							if(!f.equals("ε"))
							{
								...
								{
									lrd1 = new ProS(dA.get(j), f, 1);
								}
								else
								{
									lrd1 = new ProS(dA.get(j), f, 0);
								}					
		...
		{
			ArrayList<ProS> list = state0.getLRDs(path);//直接通过路径传到下一个状态的情况
	 ...
	 */
	private void addState(int lastState,String path,ArrayList<ProS> list)
	{
		DFA temp = new DFA(0);
				...
				for(int j = 0;j < dA.size();j++)
				{
					for(String f:firstB)
					{
						if(!f.equals("ε"))
						{
							...
							if(dA.get(j).list.get(0).equals("ε"))
							{
								lrd = new ProS(dA.get(j), f, 1);
							}
							else
							{
								lrd = new ProS(dA.get(j), f, 0);
							}	
		...
		{
			ArrayList<ProS> l = temp.getLRDs(p);//直接通过路径传到下一个状态的情况
	 ...
	 * @return
	 */
	public ArrayList<Pro> getDerivation(String v)
	{
		ArrayList<Pro> result = new ArrayList<Pro>();
		Iterator<Pro> iter = Grammar.F.iterator();
		while(iter.hasNext())
		{
			Pro d = iter.next();
			if(d.left.equals(v))
	...
	private Set<String> first(String v)
	{
		...
		else 
		{
			...
			Iterator<String> iter = Grammar.firstMap.get(v).iterator();
			while(iter.hasNext())
			{
	...
	private void createAnalyzeTable()
	{
		...
		{
			DFA state = dfa.get(i);//获取dfa的单个状态
			for(ProS lrd:state.set)
			{//对每一个进行分析
				if(lrd.index == lrd.d.list.size())
				{
					...
					{
						actionTable[i][actionIndex("#")] = Table.acc;//设为接受
					}
				} 
				else 
				{
					String next = lrd.d.list.get(lrd.index);//获取·后面的文法符号
					if(Grammar.VT.contains(next))
					{//必须是一个终结符号
	...
	private int derivationIndex(Pro d)
	{//返回是第几个表达式
		int size = Grammar.F.size();
		for(int i = 0;i < size;i++)
		{
			if(Grammar.F.get(i).equals(d))
			{
				return i;
			}
		}
