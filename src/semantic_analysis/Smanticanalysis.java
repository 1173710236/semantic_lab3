...
public class Smanticanalysis
{
    ...
    public Smanticanalysis(String filename, List<Stack<Sym>> table,
						   List<String> three_addr, List<Four> four_addr, List<String> errors)
    {
		Parser parser = new Parser(filename, tree);
		treeSize = tree.size();
		nodeSize = tree.get(treeSize-1).getFather().getId() + 1;
		tree_pro = Arrays.asList(new Properties[nodeSize]);  // 符号属性

		Smanticanalysis.three_addr = three_addr;
		Smanticanalysis.four_addr = four_addr;
		Smanticanalysis.table = table;
		Smanticanalysis.errors = errors;
    ...
    {        
    	if(table.size()==0)
    	{
    		table.add(new Stack<Sym>());
    	}
    	Sym s = new Sym(name,type,offset);
        table.get(i).push(s);  
    }  
    ...
    public static void semantic_12(Tree tree)
    {
    	...
    	three_addr.add(code);
    	four_addr.add(new Four("=",tree_pro.get(E).getAddr(),"-",id));
    ...
    public static void semantic_13(Tree tree)
    {
    	...
    	three_addr.add(code);
    	four_addr.add(new Four("=",tree_pro.get(E).getAddr(),"-",
    ...
    public static void semantic_14(Tree tree)
    {
    		...
    		(tree_pro.get(E1).getType().equals("real") && tree_pro.get(E2).getType().equals("real")))
    	{
	    	...
	    	three_addr.add(code);
	    	four_addr.add(new Four("+",tree_pro.get(E1).getAddr(),
    	...
    	if ((tree_pro.get(E1).getType().equals("real") && tree_pro.get(E2).getType().equals("integer")))
        {
	    	...
	    	four_addr.add(new Four("=","intTOreal" + tree_pro.get(E2).getAddr(),"-",newtemp1));
	    	four_addr.add(new Four("+",tree_pro.get(E1).getAddr(),newtemp1,newtemp2));
	    	//System.out.println(code); 	
        }
    	if ((tree_pro.get(E1).getType().equals("integer") && tree_pro.get(E2).getType().equals("real")))
    	{
	    	...
	    	three_addr.add(code1);
	    	three_addr.add(code2);
	    	four_addr.add(new Four("=","intTOreal" + tree_pro.get(E1).getAddr(),"-",newtemp1));
	    	four_addr.add(new Four("+",newtemp1,tree_pro.get(E2).getAddr(),newtemp2));
    	}
    	if (tree_pro.get(E1).getType().contains("array"))
    	{
	    	...
	    	three_addr.add(code1);
	    	three_addr.add(code2);
	    	four_addr.add(new Four("=",String.valueOf(x),"-",newtemp1));
	    	four_addr.add(new Four("+",newtemp1,tree_pro.get(E2).getAddr(),newtemp2));
    ...
    public static void semantic_23(Tree tree)
    {
    	...
    	if (i[0]==-1)
    	{
            ...
            four_addr.add(new Four("=",String.valueOf(4),"-",newtemp));
    	...
    	else
    	{
        	...
        	four_addr.add(new Four("=",tree_pro.get(E).getAddr(),"-",newtemp));
    	}
    	three_addr.add(code);
    	//System.out.println(code); 
    ...
    public static void semantic_24(Tree tree)
    {
    	...
    	String code2 = newtemp2 + " = " + tree_pro.get(L1).getOffset() +
    			"+" + newtemp1;
    	three_addr.add(code2);
    	four_addr.add(new Four("+",tree_pro.get(L1).getOffset(),newtemp1,newtemp2));
    ...
    public static void semantic_31(Tree tree)
    {
    	...
    	three_addr.add(code1);
    	four_addr.add(new Four("j" + tree_pro.get(R).getName(),
    			tree_pro.get(E1).getAddr(),tree_pro.get(E2).getAddr(),"-"));
    	
    	String code2 = "goto ";
    	three_addr.add(code2);
    	four_addr.add(new Four("j","-","-","-"));
    	//System.out.println(code1); 
    ...
    public static void semantic_43(Tree tree)
    {
    	...
    	three_addr.add(code);
    	four_addr.add(new Four("j","-","-",String.valueOf(tree_pro.get(M1).getQuad())));
    	//System.out.println(code); 
    }
    ...
    public static void semantic_54(Tree tree)
    {
    	...
    	for (int i=0; i<size; i++)
    	{
        	...
        	three_addr.add(code);
        	four_addr.add(new Four("param","-","-",queue.get(i)));
    	}
    	String code = "call " + id + " " + size;
    	three_addr.add(code);
    	four_addr.add(new Four("call",String.valueOf(size),"-",id));
